...
async function deploy() {
    ...
    let rawData = await faucet.constructor(
        [
            gasTotalLimit,
            collateralTotalLimit,
            gasBound,
    ...
    console.log(gasTotalLimit === generalGasTotalLimit);
    console.log(collateralTotalLimit === generalCollateralTotalLimit);
    console.log(gasBound === generalGasBound);
    console.log(storageBound === generalStorageBound);
    console.log(upperBound === generalUpperBound);
    let resData = {};
    resData.abi = config.faucet_contract.abi;
    resData.bytecode = rawData;
    fs.writeFileSync(
        __dirname + '/deployFaucet.json',
        JSON.stringify(resData),
    );
    //return;
    let tx_hash = await faucet.constructor(
        [
            gasTotalLimit,
            collateralTotalLimit,
            gasBound,
    ...
    nonce++;
    let params = await faucet.getBounds('0x0000000000000000000000000000000000000000');
    console.log(params);
    params = await faucet.getBounds('0x0000000000000000000000000000000000000001');
    console.log(params);
    //return;
    //deploy dapp
    ...
    */
    ...
    let smallBound = await faucet.getBounds('0x0000000000000000000000000000000000000000').call();
    console.log('small bounds:', smallBound);
    ...
    //update bounds
    console.log('update small bounds');
    try {
        tx_hash = await faucet.setBounds(smallKey, [0,0,0,0,0])
            .sendTransaction({
                from: new_owner,
                nonce: nonce,
                gasPrice: 1,
            });
        console.log('set smallBounds txHash:', tx_hash);
        receipt = await waitForReceipt(tx_hash);
        if(receipt.outcomeStatus !== 0) throw new Error('small set failed!');
        nonce++;
    } catch(e) {
        console.log(e);
    }
    
    faucetParams = await faucet.getBounds(smallKey).call();
    console.log('small bounds: ', faucetParams);

    console.log('update large bounds');
    try {
        tx_hash = await faucet.setBounds(largeKey, [0,0,0,0,0])
            .sendTransaction({
                from: new_owner,
                nonce: nonce,
                gasPrice: 1,
            });
        console.log('set largeBounds txHash:', tx_hash);
        receipt = await waitForReceipt(tx_hash);
        if(receipt.outcomeStatus !== 0) throw new Error('large set failed!');
        nonce++;
    } catch(e) {
        console.log(e);
    }
    
    faucetParams = await faucet.getBounds(largeKey).call();
    console.log('large bounds: ', faucetParams);
    
    console.log('update dapp3 bounds');
    try {
        tx_hash = await faucet.setBounds(dapp_addr3, [0,0,0,0,0])
            .sendTransaction({
                from: new_owner,
                nonce: nonce,
                gasPrice: 1,
            });
        console.log('set customBounds txHash:', tx_hash);
        receipt = await waitForReceipt(tx_hash);
        if(receipt.outcomeStatus !== 0) throw new Error('custom set failed!');
        nonce++;
    } catch(e) {
        console.log(e);
    }
    
    faucetParams = await faucet.getBounds(dapp_addr3).call();
    console.log('custom bounds: ', faucetParams);

    //rm from list
    console.log('rm dapp2, dapp3');
    tx_hash = await faucet.removeContract([dapp_addr2, dapp_addr3])
        .sendTransaction({
            from: new_owner,
            gas: 1000000,
            nonce: nonce,
    ...
    receipt = await waitForReceipt(tx_hash);
    if(receipt.outcomeStatus !== 0) throw new Error('dapp rm failed!');
    nonce++;
    
    //check rm result
    flag = await faucet.isLargeContract(dapp_addr2).call();
    console.log('dapp2 removed from large:', flag);
    flag = await faucet.isCustomContract(dapp_addr3).call();
    console.log('dapp3 removed from custom:', flag);
...
async function addWhiteListAdmin(address) {
    let receipt;
    let tx_hash;
    let nonce = Number(await cfx.getNextNonce(faucet_owner.address));
    ...
    tx_hash = await faucet.addWhitelistAdmin(config.faucet_newOwner)
        .sendTransaction({
            from: faucet_owner, 
            nonce: nonce,
        });
    console.log('addWhitelistAdmin txHash: ', tx_hash);
    receipt = await waitForReceipt(tx_hash);
    if(receipt.outcomeStatus !== 0) throw new Error('addWhitelistAdmin failed!'); 
}
...
if(program.admin) {
    ...
    addWhiteListAdmin(program.admin);
}
